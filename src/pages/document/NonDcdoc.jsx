...
const SaleReturnablesNonDCDocs = (props) => {
  console.log("i am from nonr doc dom " + props.newsaleordernondcId);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.29.223:8080/api/Dc/getAllDCById/${props.newsaleordernondcId}`
      );
      if (!response.ok) {
  ...
  const fetchData2 = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.29.223:8080/api/SoOrders/${props.newsaleordernondcId}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch data");
      }
...
export default SaleReturnablesNonDCDocs;
