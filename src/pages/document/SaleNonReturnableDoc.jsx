...
const SaleNonReturnablesDocs = (props) => {
  ...
  const fetchData2 = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.29.223:8080/api/SoOrders/${props.newsaleorderId}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch data");
      }
        ...
        <div className="outer-borderreturnSaleList" id="SalesOrderDoc">
          ...
          <table>
            <tbody>
              ...
              >
                <strong>NON-RETURNABLE SALES ORDER</strong>
              </h4>
...
export default SaleNonReturnablesDocs;
