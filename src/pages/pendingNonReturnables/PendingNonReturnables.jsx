...
const PendingNonReturnables = () => {
  ...
  const fetchPendingData = async (penId) => {
    try {
      const response = await fetch(
        `http://192.168.29.223:8080/api/pendings/getsaleByItems/${penId}`
      );
      if (!response.ok) {
  ...
  const handlePendingClick = (row) => {
    ...
    fetchPendingData(row.saleorderId);
  };
  ...
  return (
    <div style={{ margin: "20px" }}>
      <Header
        title="Sales Order NR Pending List"
        subtitle="Check Your Sales Order NR Pending Reports Here"
      />
      <Box
...
export default PendingNonReturnables;
